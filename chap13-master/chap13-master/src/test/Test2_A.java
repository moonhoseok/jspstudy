...
public class Test2_A {
	public static void main(String[] args) {
		Person[] arr = { new Person("홍길동", 10), new Person("김삿갓", 30),
				new Person("이몽룡", 15), new Person("김자바", 25),
				new Person("박자바", 20) };
		
		TreeSet<Person> s1 = 
			new TreeSet<Person>((p1,p2)->p1.name.compareTo(p2.name));
/*		
...
*/
		...
		TreeSet<Person> s2 = 
				new TreeSet<Person>((p1,p2)->p1.age - p2.age);
		for (Person p : arr) s2.add(p);
		System.out.print("나이순:");
		System.out.println(s2);
		
		TreeSet<Person> s3 = 
				new TreeSet<Person>((p1,p2)->p2.name.compareTo(p1.name));
		for (Person p : arr) s3.add(p);
		System.out.print("이름 역순:");
		System.out.println(s3);
		
		TreeSet<Person> s4 = 
				new TreeSet<Person>((p1,p2)->p2.age - p1.age);
